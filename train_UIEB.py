...
def main():
    ...
    RESUME = False
    checkpoint_path = "./checkpoints/UIEB7/"

    train_set = UIEBDataset("./data/", train_flag=True, pred_flag=False, train_size=hparams.crop_size,
                            input_norm=hparams.input_norm)
    test_set = UIEBDataset("./data/", train_flag=False, pred_flag=False, train_size=hparams.crop_size,
                           input_norm=hparams.input_norm)
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        filename='epoch{epoch:02d}-psnr{psnr:.4f}-ssim{ssim:.4f}',
    ...
    # 打印总运行时间
    print("Total running time: {:.4f} seconds".format(total_runtime))

    # 保存总运行时间到 txt 文件中
    runtime_txt = "total_runtime.txt"
    with open(runtime_txt, "w") as f:
        f.write("Total running time: {:.4f} seconds\n".format(total_runtime))
